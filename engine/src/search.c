...
// cannot go over time_crit - hard stop
...
#else
	frem=b->run.time_move-passed;
/*
	if((b->uci_options->movetime == 0) && (b->depth_run > 0)) {
		frem = (long long int )b->run.time_crit-passed;

		if ((frem < 0) && (b->idx_root < 1)) {
			LOGGER_4("INFO: Time out loop - move EASY, frem %lld, move: %d, crit: %d, elaps %lld, left %lld, crit left %lld, dif %d\n", frem, b->run.time_move,b->run.time_crit,passed, b->run.time_move-passed,b->run.time_crit-passed, b->search_dif );
			b->search_abort=32;
			return 32;
		}
...
{
	long long tnow, tpsd, npsd;
	long long trun, remain;
	long long int frem;
...
//			(long long int )((b->search_dif+110)/220
			...
			- trun;
#else
//	frem = (long long int )  Min(b->run.time_move * b->search_dif / 100, b->run.time_crit) - trun;
#endif
...
#else
// time_move is our target / optimal time for move
// deal with difficulties and adapt out allocated time.
//	b->search_dif=100;
long long int dyn_time = b->run.time_move * b->search_dif /100;
	frem = (long long int ) Min(dyn_time, b->run.time_crit) - trun;

// new iteration normally takes 3x of current one.
	if ((frem) < (tpsd * 4)) {
		LOGGER_4("INFO: Time out movetime - RUN1, plan: %lld, crit: %lld, iter: %lld, left: %llu, elaps: %lld, frem %lld: %lld < %lld\n", b->run.time_move, b->run.time_crit, tpsd, remain, (tnow-b->run.time_start), frem, frem*b->stats->s[S_ebfnodespri], tpsd*b->stats->s[S_ebfnodes]);
		b->search_abort=33;
		return 33;
	}
	if ((frem * 100) < ((trun + frem) * 30)) {
		LOGGER_4("INFO: Time out movetime - RUN2, plan: %lld, crit: %lld, iter: %lld, left: %llu, elaps: %lld, frem %lld\n", b->run.time_move, b->run.time_crit, tpsd, remain, (tnow-b->run.time_start), frem);
		b->search_abort=34;
		return 34;
	}
	...
	if ((b->hs != NULL)&& checks>0) {
		...
		// hash hit
			...
			if ((mt.move == NULL_MOVE) || (isMoveValid(b, mt.move, att, side, tree))) {
				if ((hash.depth > 0)) {
					tree->tree[ply][ply].move = mt.move;
					tree->tree[ply][ply].score = mt.real_score;
					if ((hash.scoretype != FAILHIGH_SC) && (mt.move != NULL_MOVE) && (mt.real_score <= talfa)) {
						b->stats->s[S_Qfaillow]++;
						b->stats->s[S_Qfailhashlow]++;
						mb = &mt;
						goto ABFINISH2;
					} else if ((hash.scoretype != FAILLOW_SC) && (mt.real_score >= tbeta)) {
						b->stats->s[S_Qfailhigh]++;
						b->stats->s[S_Qfailhashhigh]++;
						b->stats->s[S_qcutoffs]++;
						mb = &mt;
#if 1
//						b->stats->s[S_Qcutoff_long]=Max(b->stats->s[S_Qcutoff_long],0);
						b->stats->s[S_qfirstcutoffs]++;
//						if (is_quiet_move(b, att, &mt))
//							b->stats->s[S_Qquiet_cuts]++;
//						else
//							b->stats->s[S_qfirstcutoffs]++;
#endif
						goto ABFINISH2;
					} else if (hash.scoretype == EXACT_SC) {
						b->stats->s[S_Qfailhashnorm]++;
						if ((b->pers->use_hash)) {
							}
							b->stats->s[S_Qfailnorm]++;
							mb = &mt;
					...
					// hash hit, depth ok, but not over/below apropriate bound
					}
				} else {
...
#if 1
		if ((incheck)) {
			incheck2 = isInCheck_Eval(b, att, side);
			if(incheck2) {
				UnMakeMoveNew(b, &u, pos);
				rr = ChangesToMove(b, att, &u);
				generateBitmaps(b, att, rr, BLACK);
...
#endif
#if 1
		if ((checks > 0 ) && (aftermcheck))
			m->real_score = -QuiesceCheckN(b, -tbeta, -talfa,
				depth - 1, ply + 1, opside, tree, Max(checks - 1, 0),
				att);
		else
#endif
#if 0
...
#if 1
// generate checks
	if((checks>0) && (mb->real_score<talfa)&&(b->search_abort==0)&&(incheck==0) && isPV) {
...
//	int check_depth = isPV ? b->pers->quiesce_check_depth_limit : 0;
	...
	if (((ext > 0) && (ply < MAXPLY))|| (check && depth>-5)) {
...
//			if(depth>=2) {
				lmrrmoves=b->stats->s[S_movestested];
				val = -ABNew(b, -ttbeta, -talfa, Max(depth - 1,1) ,
					ply + 1, opside, tree, nulls, att);
				if (val <= talfa)
					b->stats->s[S_fhflcount]++;
				b->stats->s[S_lmrrerunnodes] += b->stats->s[S_movestested] - lmrrmoves;
...
// always talfa < ttbeta, and ttbeta should be talfa+1 or tbeta
	...
	if (ext > 0 || (check && depth>-5))
			val = -ABNew(b, -tbeta, -talfa, ext, ply + 1, opside, tree, nulls, att);
		else val = -QuiesceNew(b, -tbeta, -talfa, ext, ply + 1,
				opside, tree, b->pers->quiesce_check_depth_limit, att);
		if (val <= talfa)
			b->stats->s[S_Zfhflcount]++;
...
int can_do_LMP(board *b, attack_model *a, int alfa, int beta, int depth, int isPV, move_entry *m, move_cont *MVS, int ply, int side, uint8_t phase)
{
	int8_t from, to, rank;
	int prio, reduce;
	int see;
 ...
 */
int can_do_LMR(board *b, attack_model *a, int alfa, int beta, int depth, int isPV, move_entry *move, move_cont *MVS, int ply, int side, uint8_t phase, int seval, UNDO *u)
{
...
// king, stm low on material
	...
	int pwn=(GT_M0(b, b->pers, side, PAWN))+(GT_M0(b, b->pers, Flip(side), PAWN));
	int endg = 0;
	if((pwn<8) && (phase<128)) {
		endg=Min(3,(128-phase)/40.0 + (8-pwn)/3.0);
	}

	double dynamic_divisor = 2.3 + (double)(255 - phase) / 500.0;
	prio = checkHHTable(b->hht, side, u->old, u->to);

	if(u->whereCa != -1) {
		reduce = b->pers->lmr_table[Min(64,depth)][Min(64,move->ord+endg)] / (dynamic_divisor + 0.5);
		sval = SEE0(b, u->to, side, u->captured);
		if(sval<0) reduce+=1;
		if((sval==0) && (prio<0)) reduce++;
		if(MVS->cap_pr<3) reduce--;
	} else {
		reduce = b->pers->lmr_table[Min(64,depth)][Min(64,MVS->quiet_pr+endg)] / dynamic_divisor;
		if (prio > (HHScale/2.0)) reduce--;
		if (prio > (4*HHScale/5.0)) reduce--;
		if (prio < -(HHScale/4.0)) reduce++;
	}
// scaled value of PAWN
	int pvalue = b->pers->matval[PAWN][phase];
	if(seval < (beta - pvalue/5)) reduce--;

//	if(isPV) reduce-=2;

	if(move->phase==OTHER)	return CLAMP(reduce, 0, depth-1 );
//	if(depth>=6) return CLAMP(reduce, 0, depth-3);
...
// ply - jak jsem hluboko, 0 jsem v root pozici
{
	...
	incheck = (isInCheck_Eval(b, att, side)!=0);
	LOGGER_SE("%*d, *S , EEEE, amove ch:?, depth %d, talfa %d, tbeta %d,incheck %d\n", 2+ply, ply, depth, alfa, beta, incheck);
...
//!!!
//	incheck = (UnPackCheck(tree->tree[ply-1][ply-1].move) != 0);
//	L0("%*d, *S , EEEE, ch:%d, depth %d, talfa %d, tbeta %d,\n", 2+ply, ply, att->ke[side].attackers, depth, alfa, beta);
...
//				if(sco >= ( tbeta + b->pers->futility_cut[depth] )) {
				...
				if(sco >= ( tbeta + rsco )) {
					...
					if(b->pers->futility_sim==1) {
						L0("pval %d, fcut %d\n", pvalue, b->pers->futility_cut[depth]);
						L0("alfa %d, beta %d, talfa %d, mat_eval %d >= tbeta %d + fcut value %d, depth %d, est score %d\n", alfa, beta, talfa, sco, tbeta, rsco, depth, sco - rsco);
						futility_sim_flag=1;
					} else {
...
// null move PRUNING / REDUCING
	if ((nulls > 0) && (isPV == 0) && (b->pers->NMP_allowed > 0)
		...
		LOGGER_SE("%*d, +S , NULL, amove ch:?, depth %d, talfa %d, tbeta %d, best %d\n", 2+ply, ply, depth, talfa, tbeta, mb->real_score);
...
// engine stop protection?
		...
		&& (isMATE(mt.real_score)==0)) { //!!!!
			...
			hash.scoretype = FAILHIGH_SC;
			if ((b->hs != NULL) && (b->search_abort == 0) && depth > 0)
				storeHash(b->hs, &hash, side, ply, depth, b->norm, 
					b->stats);
...
// main loop
	LOGGER_SE("%*d, *S , XXXX, amove ch:?, depth %d, talfa %d, tbeta %d,incheck %d, inch2 %d, best %d\n", 2+ply, ply, depth, talfa, tbeta, incheck, att->ke[side].attackers!=0, mb->real_score);
//	L0("%*d, *S , XXXX, amove ch:X, depth %d, talfa %d, tbeta %d,incheck %d, best %d\n", 2+ply, ply, depth, talfa, tbeta, att->ke[side].attackers, mb->real_score);

	while (((ply==0 ? getNextRootMove(b, att, MVS, ply, side, incheck, &m, tree) : getNextMove(b, att, MVS, ply, side, incheck, &m, tree)) != 0)
		&& (b->search_abort == 0)) {

		aftermovecheck=(CheckingMove(b, att, side, m));
		...
		if(!isMoveValid(b, m->move, att, side, tree)) {
			...
			L0("invalid move!\n");
			sprintfMoveSimple(m->move, b2);
			LOGGER_0("%*d, +S , %s, ch:%d, amove ch:%d, depth %d, talfa %d, tbeta %d, best %d\n", 2+ply, ply, b2, incheck, aftermovecheck, depth, talfa, tbeta, mb->real_score);
			L0("phase:%d\n", m->phase);
...
// ze by to nefungovalo?
		if (isInCheck_Eval(b, att, opside)) {
// idea from Crafty - extend only SAFE moves
#if 1
		if (b->pers->check_extension > 0 && depth>1 && b->ext<2) {
				pval = (u.captured < ER_PIECE) ? u.captured : 0;
...
// setup window
		...
		DEB_SE(
				sprintfMoveSimple(m->move, b2);
				LOGGER_0("%*d, +S , %s, amove ch:%d, depth %d, talfa %d, tbeta %d, incheck:%d, best %d, m ord:%d, m phase:%d\n", 2+ply, ply, b2, aftermovecheck, depth, talfa, tbeta, incheck, mb->real_score, m->ord, m->phase);
		)
...
//			&& phase>=64
			&& ((u.whereCa == -1)
				|| ((m->phase>KILLER4) || (MVS->cap_pr>2 && m->phase==CAPTURES))
//				|| ((MVS->cgen>118)&&(MVS->cap_pr>2) && (m->phase==CAPTURES) && (depth ==1))
				)
			){
//!! pouzit alfa a beta ne txxxx!
			int lmr_red = can_do_LMR(b, att, talfa, tbeta, depth, isPV, m, MVS, ply, side, phase, sco, &u);
				if(lmr_red!=0) {
...
bypass2:
		...
		LOGGER_SE("%*d, -S , %s, amove ch:%d, depth %d, talfa %d, tbeta %d, incheck %d, best %d, val %d\n", 2+ply, ply, b2, aftermovecheck, depth, talfa, tbeta, incheck, mb->real_score, m->real_score);
//		L0("%*d, -S , ch:%d, depth %d, talfa %d, tbeta %d, best %d, val %d\n", 2+ply, ply, att->ke[side].attackers, depth, talfa, tbeta, mb->real_score, m->real_score);
...
// cutoff
	...
	int ddepth=CLAMP(depth,0,10);
			...
			b->stats->s[S_cutoff_cum+ddepth]+=m->ord;

#if 0
			if(b->stats->s[S_cutoff_long] < m->ord) {
				sprintfMoveSimple(m->move, b2);
				L0("Longest Cut %s at %d\n",b2, m->ord);
				printBoardNice(b);
			}
#endif
			b->stats->s[S_cutoff_long]=Max(b->stats->s[S_cutoff_long],m->ord);
			if ((m->ord == 0)) {
				b->stats->s[S_firstcutoffs]++;
//				if(MVS->cap_pr==1) b->stats->s[S_first_cap_cuts]++;
			}
			if (is_quiet_move(b, att, m)) {
				...
				if ((b->pers->use_killer >= 1)) {
					if ((m->phase>=KILLER1 && m->phase<OTHER && depth>0 && incheck==0)) {
...
// not quiet move yet not cap_pr? It is capturing/promoting hashmove!
				if(MVS->cap_pr==1 || m->ord==0) b->stats->s[S_first_cap_cuts]++;
			}
			mb = m;
//				copyTree(tree, ply);
...
//		}
	...
	if (mb->real_score > alfa && mb->real_score < beta) {
		...
		if ((b->hs != NULL) && (b->pers->use_hash == 1) && (depth > 1)
			&& (b->search_abort == 0)) {
...
ABFINISH:
	...
	b->stats->s[S_possiblemoves] += (MVS->tgen);
	if(isPV) {
		b->stats->s[S_PV_possiblemoves] += (MVS->tgen);
...
int alow, ahigh;
int pos[4];
int aspdifa[]={500, 1000, 2000, 4000, 8000, iINFINITY};
int aspdifb[]={375, 750, 1500, 3000, 6000, iINFINITY};
...
rerun:
		...
		if((b->pers->use_aspiration!=0)&&(f>=5)&&(!incheck)) {
			talfa=Max(alfa, old_score-aspdifa[alow]);
			tbeta=Min(beta, old_score+aspdifb[ahigh]);
		} else {
			talfa = alfa;
			tbeta = beta;
		}
...
// check for difficulties during interation and tune time management
		{
		int bdif=b->search_dif;
// change in best move
			if(tree->tree[ply][ply].move != b->p_pv.line[0].move) {
				b->search_dif+=10;
			}
// drop in score
			if(((best+50) < old_score) && (f > (start_depth + 1))) {
				b->search_dif+=15;
			}
			if(bdif==b->search_dif && alow==0 && ahigh==0) b->search_dif = Max(b->search_dif-10, b->run.time_move*0.65);
...
//			for(int i=0; i<tre4e->root_c.tgen; i++) tree->root_c.move[i].qorder=tree->root_c.move[i].real_score;
...
#if 1
		if ((((&tree->root_c)->count)>1) && (tree->tree[ply][ply].move!=NA_MOVE) && (best>talfa) && (best>tbeta)) {
			old_score = best;
			bestmove = tree->tree[ply][ply].move;
		} else 
#endif
...
//		(float)b->stats->s[S_ebfnodes]/(float)(b->stats->s[S_ebfnodespri]+1), (float) b->stats->s[S_ebfnodes]/(float)(tnow-tstart));
		...
		b->stats->s[S_wasted_time] += tnow-b->run.iter_start;
	}
// compute difference between start and end of an iteration
